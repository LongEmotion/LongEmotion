...
class BaseEvaluator(object):
    ...
    def __init__(self,
                 ...
                 data_dir, prompts_dir, base_dir, task):
    ...
    def run_emotionclass(self, test_data):
        ...
        output_file_root = f"{self.base_dir}/Final_result/Base/{self.task}"
        max_try=5
        time = 1
        while time <=3:
    ...
    def questionnaire(self, test_data):
        ...
        cnt=0
        while time <= 3:
            output_file=output_file_root + f"/{self.model_name}/{self.model_name}_result_{time}.jsonl"
    ...
    def run_report_summary(self, test_data):
        ...
        output_file_root = f"{self.base_dir}/Final_result/Base/{self.task}/"
        prompt = open(f"{self.prompts_dir}/gen_prompt/Emotion_Summary.txt").read()
        time = 1
