...
class SelfRAGEvaluator(BaseEvaluator):
    ...
    def critic_completion(self, query, chunk):
        ...
        prompt = format_prompt(query, paragraph=chunk)
        response = self.critic_client.completions.create(
    ...
    def run_report_summary(self, test_data):
        output_file_root = f"{self.base_dir}/Final_result/Self-rag/{self.task}/"
        ...
        while time <= 3:
            ...
            with open(output_file, 'a', encoding='utf-8') as outfile:
                for item in tqdm.tqdm(test_data, desc="Processing report summary"):
                    ...
                    print(f"full_context-tokens: {self.count_tokens(full_context)}")
                    ...
                    for dimension, query in [('causes', query_cause),
                                           ('symptoms', query_symptoms),
                                           ('treatment_process', query_process),
                                           ('characteristics_of_illness', query_characteristics),
                                           ('treatment_effect', query_effect)]:
                        retrieved_chunks = retrieval_results[dimension]
                        filtered_chunks = []
                        ...
                        for i, chunk in enumerate(retrieved_chunks):
                            ...
                            while retry_count < max_retries:
                                response = self.critic_completion(query, chunk)
                                retry_count += 1
                                if "[Partially supported]" in response or "[Fully Supported]" in response:
                                    pack = f"Chunk:{chunk}\nSummary:{response}"
                                    filtered_chunks.append(pack)
                                    break
                                elif "[No support / Contradictory]" in response or "[Irrelevant]" in response:
                                    break

                        filtered_retrieval_results[dimension] = filtered_chunks
                    ...
                    for dimension in ['causes', 'symptoms', 'treatment_process', 'characteristics_of_illness', 'treatment_effect']:
                        ...
                        filtered_chunks = filtered_retrieval_results[dimension]
                        if filtered_chunks:
                            for i, pack in enumerate(filtered_chunks):
                                retrieval_chunks_final += f"Content {i+1}:\n{pack}\n\n"
                        else:
                            retrieval_chunks_final += "No relevant content found after filtering.\n\n"
    ...
    def run_emotionclass(self, test_data):
        ...
        cnt = 0
        while time <= 3:
            output_file = output_file_root+ f"/{self.model_name}_Emo_class_{time}.jsonl"
            with open(output_file, 'a', encoding='utf-8') as outfile:
                for item in tqdm.tqdm(test_data):
                    ...
                    for i, chunk in enumerate(first_chunks):
                        response = None
                        retry_count= 0
                        query = f"What emotion(s) would {subject} ultimately feel in this situation?"
                        while retry_count < 5:
                            response = self.critic_completion(query,chunk)
    ...
    def questionnaire(self, test_data):
        ...
        output_file_root = f"{self.base_dir}/Final_result/Self-rag/Emotion_Expression/{self.model_name}/"
        ...
        case_id = -1
        output_file = output_file_root + f"{self.model_name}_result.jsonl"
                            ...
                            if stage == 1:
                                ...
                                for i, chunk in enumerate(retrieve_first):
                                    ...
                                    if "[Partially supported]" in response or "[Fully Supported]" in response:
                                        ...
                                        break
                                retrieve_first = filtered_chunks
                                # 5.最后输入
    ...
    def run_fileQA(self, test_data):
        output_file_root = f"{self.base_dir}/Final_result/Self-rag/Emotion_QA"
        ...
        cnt=0
        while time <=1 :
            output_file = output_file_root + f"/{self.model_name}_128_8_only_both.jsonl"
            with open(output_file, 'a', encoding='utf-8') as outfile:
    ...
    def run_multicov(self, test_data):
        ...
        """
        output_file_root = f"{self.base_dir}/Final_result/Self-rag/Emotion_Conversations/"
                        ...
                        for eval_label, point_idx in eval_points.items():
                            if point_idx < n_clients and point_idx >= 0:
                                ...
                                eval_response = self.chat_completion(
                                    ...
                                    messages=eval_messages,role="evaluator"
                                )
                                ...
                                try:
                                    ...
                                    print(f"Evaluation result parsing error: {e}")
    ...
    def run_emotiondetection(self, test_data):
        ...
        output_file_root = f"{self.base_dir}/Final_result/Self-rag/Emotion_Detection"
        if not os.path.exists(output_file_root):
            os.makedirs(output_file_root)
            ...
            with open(output_file, 'a', encoding='utf-8') as outfile:
                for id, item in tqdm.tqdm(enumerate(test_data), desc="Processing items"):
                    ...
                    for pack in selected_packs:
                        enrichment_query = f"What emotion(s) would the subject ultimately feel in this situation?"
                        summary = self.critic_completion(enrichment_query, pack["content"])
                        pack["summary"] = summary

                    pack_texts = [f'["index": {seg["index"]}, "text": "{seg["content"]}", "summary": "{seg.get("summary", "")}"]' for seg in selected_packs]
